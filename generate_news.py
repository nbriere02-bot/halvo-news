...
Récupère des flux RSS crypto publics, résume chaque article via l'API Anthropic
(Claude), et écrit un fichier news.json prêt à être publié à une URL statique que
l'app Halvo va simplement lire (voir NewsRepository.kt côté app).
...
=== CE QU'IL TE FAUT ===
1. Une clé API Anthropic (https://console.anthropic.com) -> variable d'environnement
   ANTHROPIC_API_KEY (ne JAMAIS la mettre en dur dans ce fichier ni la committer).
2. pip install anthropic feedparser --break-system-packages
3. Un endroit où publier le news.json généré : le plus simple est un repo GitHub
   public avec GitHub Pages activé (gratuit) — voir la section "HÉBERGEMENT" en bas.

=== EXÉCUTION AUTOMATIQUE 1X/JOUR ===
Le plus simple pour un projet solo : GitHub Actions (gratuit, pas de serveur à gérer).
Voir le fichier .github/workflows/daily_news.yml fourni à côté de ce script.
"""
...
def summarize_article(client: anthropic.Anthropic, article: dict) -> str | None:
    """Résume un article via Claude. Retourne None si l'article doit être ignoré."""
...
def main():
    ...
    for article in raw_articles:
        try:
            ...
            print(f"Erreur résumé pour '{article['title']}': {e}", file=sys.stderr)
            continue
        if summary is None:
            continue
        items.append({
